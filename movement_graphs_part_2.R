...
#  mutate(new_lat = ymin + new_lat + (eastern_front == 'East')*(1915/300)  )
...
map = ggplot()+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())  + theme(axis.text.x=element_blank(),
   ...
   geom_polygon(data = germG, aes(x = long, y = lat, group = group) ,color = "black", size = 0.1, fill = "lightgrey", linetype = 2) +
  coord_equal() 

map = map +     geom_polygon(data = locs_to_keep_shp_files_1938, aes(x = long, y = lat, group = group) ,color = "black", size = 0.1,linetype = 2, fill = "lightgrey") +
  coord_equal()+
  coord_sf(xlim = c(-5,50), ylim = c(30,70)) + theme_minimal()+ geom_vline(xintercept = seq(-100,100,2), linetype = 1, alpha = .25) + 
  geom_hline(yintercept = seq(-100,100,2), linetype = 1, alpha = .25) 

map
##
  annotate("rect", xmin = xmin, xmax = xmax, ymin = 00, ymax = 29,
           alpha =1,fill = "white")+geom_sf()  + 
  geom_rect(mapping=aes(xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=30.25), fill="white", alpha=1) +
  scale_size(range = c(0,.01)) + 
  geom_text(data = text_dates, aes(x = x_loc, y = -2, label = text, angle = 90))+
  geom_text(data = y_scales, aes(x = 0, y = y_loc, label = text)) + xlab('')+ylab('')+
  
  xlim(xmin,xmax) + theme_minimal() + ylim(-3,ymax)+scale_colour_manual(values=luft_pallette)+  
  geom_text(aes(x = 20, y = 28, label = 'Number of Planes Lost By Month \n(Loss Data Available 1942-1944)'))+
...
map +
  ...
  geom_path(data= to_maps %>% select( -numeric_group_type_graph), 
            aes(x = new_lon , y = new_lat, group = plocation_order, 
                colour = group_type_graph, alpha = .25 , size =.01, shape = group_type_graph), size =.01)
...
map +
  annotate("rect", xmin = xmin, xmax = xmax, ymin = 00, ymax = 29,
           alpha =1,fill = "white")+
  geom_rect(mapping=aes(xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=30), fill="white", alpha=1) +
  geom_segment(data = geom_segment_data%>%  subset(date_start < '1943-06-15' ), aes(x = start_lon, y = start_lat, xend = end_lon, 
                                                                                                                 yend = end_lat, group = plocation_order, colour = group_type_graph, size = .2), size = .2, alpha = .1)+
  scale_size(range = c(0,.01)) + facet_wrap(date_start~.)
...
animation = map +
  ...
  geom_point(data = rbind(to_maps %>% select( -numeric_group_type_graph, - fighters), lost_to_maps ) %>% 
               mutate(date_start = (date_start)) %>% arrange(date_start) %>%  subset(date_start > '1942-05-15'  & date_start < '1942-12-15' )
  ...
  geom_text(data = text_dates, aes(x = x_loc, y = -2, label = text, angle = 90))+ xlab('')+ylab('')+
  ...
  exit_disappear()+
  theme(legend.position = "None") +ease_aes('linear', interval = 0.01,state_length = 1)+
...
animation = map +
  ...
  geom_point(data = rbind(to_maps %>% select( -numeric_group_type_graph), lost_to_maps) %>% 
                                                             mutate(date_start = (date_start)) %>% arrange(date_start) %>%  subset(date_start > '1942-02-14' & date_start < '1943-09-14'  )
             ,aes(x = new_lon , y = new_lat  , group = plocation_order, colour = group_type_graph, 
                                                                alpha = 1 , size =.1), inherit.aes = FALSE) +
  scale_size(range = c(0,.01)) + 
  #geom_point(data = luftwaffe_locations%>%  subset(date_start > '1942-02-14' & date_start < '1944-12-14'  ) %>% select(date_start, lon, lat) 
  ...
  transition_time((date_start )) + 
  #transition_reveal(date_start) + 
  exit_disappear()+theme(legend.position = "None") +ease_aes('linear', interval = 0.01,state_length = 1)+
  labs(title = paste('Monthly Movements and Losses of the Luftwaffe',' (Month): {frame_time}'))+
  scale_alpha(guide = 'none') + theme(text = element_text(size = 20)) + 
  theme(plot.title = element_text(hjust = 0.5))
animation
...
temp_move_plot = animate(animation, 
                         duration = 30*5, 
                         fps  =  30, height =PLOT_WIDTH*(ymax - ymin)/(xmax-xmin) , width = PLOT_WIDTH)
temp_move_plot = animate(animation, 
                         duration = 30, 
                         fps  =  30, height =2338 , width = 3600)

temp_move_plot
##
2,340x1,080 
anim_save(filename = "/users/sweiss/src/luftwaffe_locations/plots/mid_war_prototype3.gif", temp_move_plot)
...
test_permant_df = test_permant_df %>% filter( is.na(exclude)) %>% select(-exclude)
lost_to_maps_1 = rbind(lost_to_maps, test_permant_df)
lost_to_maps = lost_to_maps_1 %>% mutate(lat = lat-30, new_lat = new_lat-30)
lost_to_maps_1= lost_to_maps_1 %>% mutate(lost_to_maps, new_lat = new_lat)
