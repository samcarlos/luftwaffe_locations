...
ggplot(data = world_map, color = 'black', fill = 'blue') +geom_sf() + 
  geom_point(data = to_maps %>% mutate(date_start = date_start %m-% months(1)) %>% 
             subset(date_start == '1940-01-01'), 
             aes(x = new_lon, y = new_lat, colour = group_type_graph, size = .01, alpha = .2)) + 
  scale_size(range = c(0,.1)) + scale_alpha(range = c(0,.5))+ theme_bw() + 
...
get_movement_plots = function(start_date, end_date, xlims, ylims, text_title){
  ...
  temp_move_plot = ggplot(data = world_map, color = 'grey') + xlim(xlims) + ylim(ylims)+
    geom_sf() + geom_point(data =   to_maps %>% select( -numeric_group_type_graph) %>% 
                           ...
                           aes(x = new_lon , y = new_lat  , group = plocation_order, colour = Plane_Type, alpha = .9 , size =.1), inherit.aes = FALSE)  + 
    scale_size(range = c(0,.01), guide = 'none')  +scale_alpha(range = c(0,.5), guide = 'none')+ theme_minimal() +
    transition_time((date_start )) + exit_disappear()+theme(legend.position = "bottom") +ease_aes('linear', interval = 0.01,state_length = 1)+
    ...
    scale_colour_manual(values=cbPalette)+theme(plot.title = element_text(hjust = 0.5))
...
mid_war = get_movement_plots('1942-03-10' ,'1945-02-10',
