...
location_sizes_map_front_prev_uncounted_lost = location_sizes_map_front_prev_uncounted_lost %>% mutate(lon_to_go = ((lon_to_go - as.numeric(as.Date('1939-09-15') )))) %>% mutate(lon_to_go = lon_to_go/(as.numeric(as.Date('1945-05-15') )-as.numeric(as.Date('1939-09-15') )))
...
location_sizes_map_front_prev_uncounted_lost = location_sizes_map_front_prev_uncounted_lost %>% group_by( date_start) %>% mutate(plocation_order = 1:n()) %>%
  mutate(new_lat = plocation_order/100) %>% as.data.frame()
...
to_maps = to_maps %>% mutate(numeric_group_type_graph =as.numeric(as.factor(group_type_graph))) %>%
  ...
  mutate(new_lat = lat + ((location_order) %% 100 )/50   ,
         new_lon =  lon + floor(location_order/100) / 5+.1  ,
...
map = ggplot() +
  ...
  coord_equal()
map = map +     geom_polygon(data = locs_to_keep_shp_files_1938, aes(x = long, y = lat, group = group) ,color = "black", size = 0.1, fill = "lightgrey") +
  coord_equal()+
...
map = map +
  ...
  geom_text(data = y_scales, aes(x = 0, y = y_loc, label = text)) + xlab('')+ylab('')+
   ...
   theme_minimal() + coord_sf(xlim = c(xmin,xmax), ylim = c(-3,ymax)) +
...
animation = map +
  ...
  geom_point(data =   rbind(to_maps %>% select(colnames(lost_to_maps)), lost_to_maps) %>% 
                    mutate(date_start = as.Date(date_start)) %>% 
                                                             mutate(date_start = (date_start)) %>% arrange(date_start) %>%  subset(date_start > '1942-02-14' & date_start < '1943-01-14'  )
             ,
                                                           aes(x = new_lon , y = new_lat  , group = plocation_order, colour = group_type_graph, 
                                                                alpha = .9 , size =.1), inherit.aes = FALSE) +
  scale_size(range = c(0,.01)) + 
  ...
  coord_sf(xlim = c(xmin,xmax), ylim = c(-3,ymax)) + theme_minimal()+scale_colour_manual(values=luft_pallette)+  
  geom_text(aes(x = 20, y = 28, label = 'Number of Planes Lost By Month \n(Loss Data Available Mid 1942-1945)'))+
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) +
...
temp_move_plot = animate(animation, 
                         duration = 3*15, 
                         fps  =  30, height =PLOT_WIDTH*(ymax + 3)/(xmax-xmin) , width = PLOT_WIDTH)
temp_move_plot
##

anim_save(filename = "/users/sweiss/src/luftwaffe_locations/plots/mid_war_prototype2.gif", temp_move_plot)
