...
location_sizes_map_front_prev_uncounted = location_sizes_map_front_prev_1 %>% uncount(ceiling(total_w_approx/10))
location_sizes_map_front_prev_uncounted_lost = location_sizes_map_front_prev_1 %>% uncount(ceiling(plot_lost/10))
...
to_maps = to_maps %>% mutate(numeric_group_type_graph =as.numeric(as.factor(group_type_graph))) %>%
  group_by(date_start,group_type_graph,lat, lon) %>% arrange(lat, lon,group_type_graph,date_start) %>% 
  mutate(location_order = 1:n()) %>% 
  mutate(new_lat = location_order/10  + lat ,
         new_lon =  numeric_group_type_graph + lon) %>%
  as.data.frame()
...
animation = ggmap(map) +
  ...
  geom_point(data =  rbind(to_maps %>% select( -numeric_group_type_graph), lost_to_maps) %>% 
               ...
               subset(date_start > '1942-04-14' & date_start < '1943-08-14')
               ,
             aes(x = new_lon , y = new_lat  , group = plocation_order, colour = group_type_graph, 
                 shape = as.factor(group_type_graph), alpha = .5 )) + geom_point(alpha = 1/10)+scale_size(range = c(0,2)) + 
...
animate(animation, 
          duration = 30, # = 365 days/yr x 3 years x 0.25 sec/day = 274 seconds
          fps  =  30)
...
luftwalocation_sizes_map_front_prev[,'map_front'] = factor(location_sizes_map_front_prev[,'map_front'], levels = map_front_order )
...
x_coord_white_space = 10
x_coord_graph_space = 10
...
num_aircraft_by_map_front = merge(num_aircraft_by_map_front, subset(location_sizes_map_front_2, group_type_graph !='') %>% group_by(map_front, date_start) %>% summarise(num_aircraft = sum(total_w_approx)) %>%
  group_by(map_front) %>% summarise(max_ac_month = max(num_aircraft)) %>% as.data.frame() , on = 'map_front')
...
#num_aircraft_by_map_front = num_aircraft_by_map_front %>% mutate(white_space_vertical = 0) %>% arrange(date_start, map_front) %>% group_by(date_start) %>%
#  mutate(height = cumsum(num_aircraft + white_space_vertical), height_2 = cumsum(max_ac_month)) %>% as.data.frame()
...
get_barplot_date_map = function(temp_data){
  ...
  temp_data %>% 
    ...
    geom_text(data = temp_data, aes(x = factor(group_type_graph_name),  y=total_w_approx+5, label = group_type_graph_name), 
              position = position_dodge(0.9), size = 3, angle = 90,  color = "Black", hjust = 'left')+
    ...
    theme(legend.position="none",axis.ticks.x=element_blank(),plot.margin = margin(), axis.ticks.length.x = unit(0,'pt'))  +
...
temp_plots = lapply(1:nrow(num_aircraft_by_map_front),
                    ...
                    function(x){
                      ...
                      plot = merge(grop_type_graph_names, location_sizes_map_front_2, by = 'group_type_graph') %>%
                        subset(date_start == num_aircraft_by_map_front[x,c('date_start')] & map_front == num_aircraft_by_map_front[x,c('map_front')]) %>%
...
df <- data.frame(x = c((1010),max(num_aircraft_by_map_front[,'x_coord_end'])),
...
#     
#     ggsave(temp_plots[[x]], file = 'plots/gif_plots/temp.png', 
#            width = 10*100,
#            height = (num_aircraft_by_map_front[x,'end_height']-num_aircraft_by_map_front[x,'start_height'])*100, units = 'px')    
#     
#     base = base +  annotation_custom(grob = rasterGrob(image_read('plots/gif_plots/temp.png')),
...
for( x in 1:300){
  if((num_aircraft_by_map_front[x,'date_start'] > '1939-08-01' )& (num_aircraft_by_map_front[x, 'num_aircraft'] > 1.6 )){
    base = base +  annotation_custom(grob = ggplotGrob(temp_plots[[x]]),
                                     ymin = num_aircraft_by_map_front[x,'start_height'],
                                     ymax = num_aircraft_by_map_front[x,'end_height'],
                                     xmin = num_aircraft_by_map_front[x,'x_coord_start'],
                                     xmax = num_aircraft_by_map_front[x,'x_coord_end']) 
...
top_movements = (location_sizes_map_front_prev %>%
                   ...
                   mutate(curve_heights = sign( distance_to_e_germany_plot_prev - distance_to_e_germany_plot)*total_w_approx ))  %>% subset(total_w_approx >50)
...
# timeline_text = timeline_text %>% mutate(y_start = ifelse(east == "East", max_height+10, min(df[,'y'])+10 ))
# timeline_text = timeline_text %>% mutate(y_end = ifelse(east == "East", max((df[,'y'])), min_height-10  ))
...
#ggsave(base, file="plots/main_plot_2.png",width = 20000, height = 6000, units = "px",limitsize = FALSE)
ggsave(base, file="plots/main_plot_2.png",width = 30000, height = 20000, units = "px",limitsize = FALSE)


###

get_map_date_subsets = function(start_date, end_date){
  
  num_aircraft_by_map_front_temp = num_aircraft_by_map_front %>% subset(date_start >= start_date & date_start < end_date)
  
  to_subtract_x = min(num_aircraft_by_map_front_temp[,'x_coord_start'])
  
  num_aircraft_by_map_front_temp  = num_aircraft_by_map_front_temp %>%
    mutate(x_coord_start = x_coord_start - to_subtract_x, x_coord_end = x_coord_end - to_subtract_x)
  
  df <- data.frame(x = c((0),max(num_aircraft_by_map_front_temp[,'x_coord_end'])),
                   y = c(min(num_aircraft_by_map_front_temp[,'start_height']) *1.5,max(num_aircraft_by_map_front_temp[,'start_height'])*1.5))
  ...
  temp_plots = lapply(1:nrow(num_aircraft_by_map_front_temp),
                      
                      function(x){
                        
                        plot = merge(grop_type_graph_names, location_sizes_map_front_2, by = 'group_type_graph') %>%
                          subset(date_start == num_aircraft_by_map_front_temp[x,c('date_start')] & map_front == num_aircraft_by_map_front_temp[x,c('map_front')]) %>%
                          get_barplot_date_map
                        return(plot)
  ...
  for( x in 1:length(temp_plots)){
    if((num_aircraft_by_map_front_temp[x,'date_start'] > '1939-08-01' )& (num_aircraft_by_map_front_temp[x, 'num_aircraft'] > 1.6 )){
      base = base +  annotation_custom(grob = ggplotGrob(temp_plots[[x]]),
                                       ymin = num_aircraft_by_map_front_temp[x,'start_height'],
                                       ymax = num_aircraft_by_map_front_temp[x,'end_height'],
                                       xmin = num_aircraft_by_map_front_temp[x,'x_coord_start'],
                                       xmax = num_aircraft_by_map_front_temp[x,'x_coord_end']) 
  ...
  top_movements = (location_sizes_map_front_prev %>%
                     ...
                     summarise(total_w_approx = sum(total_w_approx)) %>% as.data.frame() %>%
                     mutate(curve_heights = sign( distance_to_e_germany_plot_prev - distance_to_e_germany_plot)*total_w_approx ))  %>% subset(total_w_approx >50)
  top_movements_temp = top_movements %>% subset(date_start >= (as.Date(start_date)+months(1)) & date_start < as.Date(end_date) )
  
  for( x in  1:nrow(top_movements_temp) ){
    
    end_data = subset(num_aircraft_by_map_front_temp, map_front == top_movements_temp[x,'map_front'] & date_start == top_movements_temp[x,'date_start'])
    start_data = subset(num_aircraft_by_map_front_temp, map_front == top_movements_temp[x,'prev_map_front'] & date_start == top_movements_temp[x,'date_start']-months(1))
    ...
    y_end = end_data %>% mutate(avg_height = (end_height + start_height)/2 ) %>% pull(avg_height)
    x_end = end_data %>% pull(x_coord_start)
    temp_df_start_end = data.frame(y_start, x_start, y_end, x_end, size = top_movements_temp[x,'total_w_approx'])
    map_front_adjustment_y = (max(as.numeric(num_aircraft_by_map_front_temp[,'map_front'])) - as.numeric(start_data[,'map_front'])) /max(as.numeric(num_aircraft_by_map_front_temp[,'map_front']))*(max_height_graph/2)
    print(map_front_adjustment_y)
    temp_df_start_end = data.frame(y_start, x_start,  y_end , x_end, size = top_movements_temp[x,'total_w_approx'])
  ...
  date_locs = num_aircraft_by_map_front_temp %>% group_by(date_start) %>% summarise(start_x = min(x_coord_start), end_x = max(x_coord_end)) %>% as.data.frame()
  for(x in 1:nrow(date_locs)){
...
chart_1 = get_map_date_subsets("1939-08-01","1941-06-01")
chart_2 = get_map_date_subsets("1941-06-01", "1943-05-01")
chart_3 = get_map_date_subsets("1943-05-01","1945-05-01")

#ggsave(base, file="plots/main_plot_2.png",width = 20000, height = 6000, units = "px",limitsize = FALSE)
ggsave(chart_1, file="plots/main_plot_seperate_1.png",width = 30000, height = 20000, units = "px",limitsize = FALSE)
ggsave(chart_2, file="plots/main_plot_seperate_2.png",width = 30000, height = 20000, units = "px",limitsize = FALSE)
ggsave(chart_3, file="plots/main_plot_seperate_3.png",width = 30000, height = 15000, units = "px",limitsize = FALSE)

chart_1_2 = get_map_date_subsets("1941-06-01",  "1942-07-01")
chart_1_1 = get_map_date_subsets( "1942-07-01","1945-05-01")
big_chart_2 = get_map_date_subsets( "1939-08-01","1945-05-01")
#ggsave(base, file="plots/main_plot_2.png",width = 20000, height = 6000, units = "px",limitsize = FALSE)
ggsave(chart_1_2, file="plots/main_plot_seperate_1_2.png",width = 36, height = 24, units = "in",dpi = 300,limitsize = FALSE)
ggsave(chart_2_2, file="plots/main_plot_seperate_2_2.png",width = 36, height = 24, units = "in",dpi = 300,limitsize = FALSE)
ggsave(big_chart_2, file="plots/main_plot_big_chart_2.png",width = 40000, height = 6000, units = "px",limitsize = FALSE)
ggsave(chart_1_1, file="plots/main_plot_seperate_1_1_test.png",width = 36, height = 24, units = "in",dpi = 300,limitsize = FALSE)
